  ...
  *result_size = 0;
  ...
  while (string[i]) {
    while (string[i] == pattern[j]) {
      if (j == pattern_len - 1) {
        // We've reached the end of a pattern, copy from offset to
        // current index, minus the pattern
        memcpy(result[*result_size], string + offset,
               i - offset - pattern_len + 1);
        offset = i + 1;
        *result_size += 1;
        break;
      }
...
void *insertion_sorted(void *items, int num_items, int item_size,
                       cmp_func cmp) {
  void *items_copy, *tmp, *curr, *compare;
  // Void Pointer Arithmetic Is Bad
  char *vpaib;
